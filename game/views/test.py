...
from .player_panel.new_level import set_new_level
...
def test(request):
    player = Player.objects.get(pk=103)

    res = firstInvestToCommandBusiness(player)

    response = JsonResponse({"firstInvestToCommandBusiness": res})
    return response
